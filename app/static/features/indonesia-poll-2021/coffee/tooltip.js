...
const uppers =
  /^([A-Z]{2,}|University|Australian*|China|Chinese|United States|India[n]*|Indonesia[n]*|Russia|Japan[ese]*|English|Taiwan[ese]*|Hong Kong|North Korea[n]*|United Kingdom|Xi|Jacinda|Vladimir|Scott|Boris|Anthony|Kim|Joko|Narendra|G20|Malaysia|Philippine|Myanmar|Singapore[an]*|Netherlands|Saudi Arabia[n]*|United Arab Emirates|Tentara|Badan|Kementerian|Kepolisian|Asia|Indo|Southeast Asia)\b/
...
const formatPhrase = (str) => {
  return str
    ...
    .replace(/\bUS\b/g, "the United States")
    .replace(/\bUK\b/g, "the United Kingdom")
    .replace(
      /(^United States|^United Kingdom|^media|Asia-Pacific|Indo-Pacific)/,
      "the $1"
    )
...
const setPhrase = (el) => {
  ...
  const { phrase, variant } = tooltip.phrases[el.dataset.tooltip]
...
const initialise = () => {
  tooltip.targets.forEach((el) => setPhrase(el))
...
window.addEventListener("DOMContentLoaded", () => {
  window.dispatchEvent(new CustomEvent("initCharts"))
  tooltip.initialise()
  console.log("Initialised all charts")
  resize()
})
